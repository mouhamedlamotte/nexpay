...
@ApiTags('Settings > Webhook Configuration > Wave')
@Controller('settings/webhook/wave')
export class WaveWebhookConfigController {
  private readonly logger = new Logger(WaveWebhookConfigController.name);

  constructor(
    private readonly service: WaveWebhookConfigService,
    private readonly omConfig: OMWebhookConfigService,
  ) {}
